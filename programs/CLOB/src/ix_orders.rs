...
pub fn place_order_handler(ctx: Context<PlaceOrder>, params: PlaceOrderParams) -> Result<()> {
    ...
    require!(!oo.active, OrdersError::AlreadyActive);
...
fn ensure_book_boot<'info>(
    ...
    if ai.data_len() < Blob::LEN + slab::SlabHeader::LEN {
        let first = core::cmp::min(BOOK_BOOT_NODES, core::cmp::max(max_cap, 1));
        let want = Blob::LEN + slab::region_len_for(first);
        ensure_funded_resize(payer, sys_prog, ai, want)?;
        slab::init(ai, first)?;
    }
    Ok(())
}
