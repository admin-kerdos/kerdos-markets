...
) => BigInt(await conn.getBalance(pk, "finalized"));

describe("place/cancel without matching", () => {
  it("locks lamports then refunds on cancel", async () => {
    const user = Keypair.generate();
    const userWallet = new anchor.Wallet(user);
    const userProvider = new anchor.AnchorProvider(
      baseProvider.connection,
      userWallet,
      { ...baseProvider.opts, commitment: "finalized", preflightCommitment: "finalized" }
    );
    ...
    try {
      ...
      const initSig = await program.methods
        ...
        .rpc();
      await userProvider.connection.confirmTransaction(initSig, "finalized");
      ...
      const placeSig = await program.methods
        ...
        .rpc();
      await userProvider.connection.confirmTransaction(placeSig, "finalized");
      ...
      const cancelSig = await program.methods
        ...
        .rpc();
      await userProvider.connection.confirmTransaction(cancelSig, "finalized");
